...
- `~/.github/workflows/agent-harness-parity.yml` and `~/.github/workflows/check.yml` — the actual 3 CI jobs (`parity` on `ubuntu-latest`; `lint` and `test` in `check.yml`, `test` on `macos-latest`) this skill's default heuristics were tuned against. Re-read these if the CI topology has changed since 2026-08-02.
