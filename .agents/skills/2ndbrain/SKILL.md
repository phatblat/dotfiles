...
Use this skill when the user asks to quick-save/save the last response to 2ndBrain,
or invokes `$2ndbrain` or `/skill:2ndbrain`.

Quick-captures the assistant's most recent reply (the one immediately before this
skill was invoked) as a standalone note in `~/2ndBrain/quick-notes/<year>/`.
...
The arguments supplied with the skill invocation provide a title override and/or
extra tags, e.g. `/skill:2ndbrain Redis eviction policy notes`. If omitted,
generate a short 3-6 word title that summarizes the captured response.
...
running this skill. Do **not** pick the most recently modified `*.jsonl`:
...
session running this skill. Identify the transcript that contains *this*
invocation instead.
...
invocation**, not the last one in the file. This skill runs mid-turn: by the
...
- If arguments are supplied with the skill invocation, use them verbatim as the title.
