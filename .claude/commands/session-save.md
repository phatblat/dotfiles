...
3. `---` separator
4. Full session transcript — formatted as `**User**: ...` / `**Claude**: ...` blocks with tool calls noted inline
...
- Designed and wrote the /session-save command to ~/2ndBrain/claude-sessions/.
```
...
```
vault=~/2ndBrain
out_dir="${vault}/claude-sessions/${session_year}"
...
```
Session saved: ~/2ndBrain/claude-sessions/<year>/<filename>
