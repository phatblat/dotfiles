            ...
            "python3 scripts/agent-harnesses.py validate",
            {
                **complete_shared(["claude", "codex", "opencode", "pi"]),
                ...
                "crush": state(
                    "complete",
                    "native",
                    [display_path(CRUSH / "shared-harness.md")],
                    "inlined shared harness instructions + compaction contract + commit attribution; loaded through option global-context-path in the generated crushrc",
                ),
            },
        ),
        feature(
            ...
            "python3 scripts/agent-harnesses.py inventory --json",
            {
                **complete_shared(["claude", "codex", "opencode", "pi"]),
                ...
                "crush": state(
                    ...
                    "native",
                    [display_path(SKILL_SOURCE)],
                    "generated crushrc adds ~/.agents/skills through option skill-path; crush's default global skill roots are ~/.config/crush/skills and ~/.config/agents/skills, so the shared root is named explicitly",
                ),
            },
        ),
        feature(
            ...
            "python3 scripts/agent-harnesses.py inventory --json",
            {
                ...
                "crush": state(
                    ...
                    "native",
                    [display_path(SKILL_SOURCE)],
                    "crush has no user-defined slash-command surface, so the command workflows are reachable only through the shared skills crush discovers; crush's user-invocable skill frontmatter cannot be set without forking the shared SKILL.md",
                    "Re-check crush releases for a user-defined command surface, or teach the shared skill renderer to emit crush-specific user-invocable frontmatter",
                ),
            },
        ),
        feature(
            ...
            "python3 scripts/agent-harnesses.py inventory --json",
            {
                ...
                "crush": state(
                    ...
                    "native",
                    [display_path(SHARED / "agents")],
                    "crush ships exactly two built-in agents (coder, task) and has no user-defined agent surface; upstream tracks this as charmbracelet/crush#3269",
                    "Re-check crush releases for user-defined agent support",
                ),
            },
        ),
        feature(
            ...
            "python3 scripts/agent-harnesses.py verify",
            {
                **complete_adapter(["claude", "codex", "opencode", "pi"]),
                ...
                "crush": state(
                    ...
                    "adapter",
                    [
                        display_path(CRUSH / "crushrc"),
                        display_path(CRUSH / "hooks" / "harness-guard.py"),
                    ],
                    "generated crush PreToolUse guard maps crush's snake_case payload to the shared guard and blocks with exit 2, but crush is not installed locally so live blocking is unverified",
                    "Install crush and verify it blocks a denied shell command through the generated crushrc PreToolUse hook",
                ),
            },
        ),
        feature(
            ...
            "python3 scripts/agent-harnesses.py verify",
            {
                ...
                "crush": state(
                    ...
                    "native",
                    [display_path(CRUSH / "shared-harness.md")],
                    "compaction preservation contract inlined in the generated crush context file",
                ),
            },
        ),
        feature(
...
def render_manifest_markdown(manifest: dict[str, Any]) -> str:
    ...
    lines.extend(render_attribute_table(manifest["attribute_mappings"]))
    lines.extend(
        [
            "",
            "### Mapping Sources",
